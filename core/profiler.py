...
class PerformanceMonitor:
    ...
    def stop_monitoring(self) -> dict[str, float]:
        ...
        logger.debug(
            f"Performance monitoring stopped. Memory: {memory_usage:.2f}MB, CPU: {cpu_usage:.2f}%"
        )
...
class AlgorithmProfiler:
    ...
    def profile_algorithm(
        ...
        logger.info(
            f"Starting profiling for {algorithm_name} with input size {input_size}"
        )
        ...
        try:
            ...
            logger.info(
                f"Profiling completed for {algorithm_name}. "
    ...
    def profile_function(
        ...
        try:
            ...
            execution_time = (
                end_time - start_time
            ) / iterations  # Average time per iteration
            ...
            profiling_result = ProfilingResult(
                ...
                metadata={
                    "function_result": (
                        str(first_result) if "first_result" in locals() else None
                    )
                },
            )

            logger.info(
                f"Function profiling completed for {algorithm_name}. "
    ...
    def compare_algorithms(
        ...
        for i, (algorithm, name, params) in enumerate(
            zip(algorithms, algorithm_names, parameters_list, strict=False)
        ):
            logger.info(f"Comparing algorithm {i+1}/{len(algorithms)}: {name}")
            result = self.profile_function(
                algorithm, name, input_data, params, iterations
            )
        ...
        for result in results:
            logger.info(
                f"  {result.algorithm_name}: {result.execution_time:.4f}s "
