...
from workers.tasks import register_worker_task, health_check_task
        ...
        while self.running:
            try:
                ...
                if health_info.get("status") == "healthy":
                    logger.debug(f"Health check passed: CPU {health_info.get('cpu_usage', 0):.1f}%")
                else:
                    logger.warning(f"Health check failed: {health_info.get('error', 'Unknown error')}")
